...
trayclient::trayclient(connection& conn, xcb_window_t win) : m_connection(conn), m_window(win) {
...
void trayclient::configure_notify(int16_t x, int16_t y, uint16_t w, uint16_t h) {  // {{{
 ...
 */
int traymanager::calculate_client_xpos(const xcb_window_t& win) {  // {{{
 ...
 */
int traymanager::calculate_client_ypos() {  // {{{
 ...
 */
shared_ptr<trayclient> traymanager::find_client(const xcb_window_t& win) {  // {{{
  for (auto&& client : m_clients)
    if (client->match(win)) {
...
void traymanager::track_selection_owner(xcb_window_t owner) {  // {{{
  try {
    ...
    m_connection.change_window_attributes_checked(owner, XCB_CW_EVENT_MASK, value_list);
  } catch (const xpp::x::error::window& err) {
...
void traymanager::process_docking_request(xcb_window_t win) {  // {{{
  ...
  try {
    ...
    const uint32_t value_list[]{XCB_BACK_PIXMAP_PARENT_RELATIVE,
        XCB_EVENT_MASK_PROPERTY_CHANGE | XCB_EVENT_MASK_STRUCTURE_NOTIFY};
    m_connection.change_window_attributes_checked(
        client->window(), XCB_CW_BACK_PIXMAP | XCB_CW_EVENT_MASK, value_list);
